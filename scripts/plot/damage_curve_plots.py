...
def main(config):
    ...
    damage_data_keys = pd.read_csv(os.path.join(damage_data_path,'asset_damage_curve_mapping.csv'))
    ...
    for data_key in damage_data_keys.itertuples():
        ...
        if 'damage_ratio_1' in data.columns.values.tolist():
            ...
            if 'damage_ratio_3' in data.columns.values.tolist():
                ax.plot(x_data,
                    data['damage_ratio_3'],'-',
                    color='#3182bd',linewidth=2.0,
                    label=f'{data_key.source_3} curve')
        else:
            ax.plot(x_data,
                    data['damage_ratio'],'-',
                    color='#3182bd',linewidth=2.0,
        ...
        ax.fill_between(x_data,
                        ...
                        alpha=0.3,facecolor=fill_color,
