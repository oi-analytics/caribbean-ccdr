...
def estimate_time_series(hazard_data_details,
    ...
    baseline_df = hazard_data_details[hazard_data_details.rcp == "baseline"]
    future_df = hazard_data_details[hazard_data_details.rcp != "baseline"]
    
    hazard_rcp_rp = list(
                        set(
                            zip(
                                future_df.hazard.values.tolist(),
    ...
    for ix, (haz,rcp,rp) in enumerate(hazard_rcp_rp):
        f_df = future_df[(future_df.hazard == haz) & (future_df.rcp == rcp) & (future_df.rp == rp)]
        b_df = baseline_df[(baseline_df.hazard == haz) & (baseline_df.rp == rp)]
        b_df["rcp"] = rcp
...
def main(config,country,
    ...
    goals_df = pd.read_excel(os.path.join(processed_data_path,
                    ...
                    sheet_name=f"{country}_resilience_goals")
    ...
    output_file = os.path.join(combined_results,
                            f"{country}_all_assets_risks_investments_with_resilience_goals.xlsx")
    ...
    for hk in ["min","mean","max"]:
        ...
        combined_results_df[f"avoided_damage_{hk}"] = np.where(
                        combined_results_df[f"damage_{hk}_as_gdp_percentage"] > gdp_target,
                        combined_results_df[f"avoided_damage_{hk}"] + (
                            1 - gdp_target/combined_results_df[f"damage_{hk}_as_gdp_percentage"]
                            )*combined_results_df[f"damage_{hk}"],combined_results_df[f"avoided_damage_{hk}"])
        combined_results_df[f"damage_{hk}"] = np.where(
                        combined_results_df[f"damage_{hk}_as_gdp_percentage"] > gdp_target,
                        (gdp_target/combined_results_df[f"damage_{hk}_as_gdp_percentage"]
                            )*combined_results_df[f"damage_{hk}"],
                        combined_results_df[f"damage_{hk}"])
        combined_results_df[f"adaptation_investment_{hk}"] = np.where(
                        combined_results_df[f"damage_{hk}_as_gdp_percentage"] > gdp_target,
                        (combined_results_df[f"damage_{hk}_as_gdp_percentage"]/gdp_target
                            )*combined_results_df[f"adaptation_investment_{hk}"],
                        combined_results_df[f"adaptation_investment_{hk}"])
        combined_results_df[
                f"damage_{hk}_as_gdp_percentage"
                ] = 100.0*combined_results_df[f"damage_{hk}"]/combined_results_df["gdp"]
